...
export default function Navbar({
  ...
  return (
    <>
      <header className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[92%] max-w-6xl pointer-events-auto overflow-visible">
        <nav
          className={`floating-dock ${scrolled ? 'scrolled' : ''} rounded-full px-5 sm:px-6 py-2.5 sm:py-3 transition-all duration-300 overflow-visible`}
        ...
        >
          <div className="flex items-center justify-between gap-3 sm:gap-6">
            ...
            {/* Logo */}
            <a
              className="brand-mark no-underline font-bold text-lg brand-logo-link"
              href="#"
              onClick={(e) => { e.preventDefault(); onNavigate('home'); }}
              data-testid="brand-home"
            >
              <img src="/logo.svg" alt="Veritas+ Logo" className="brand-logo-img" />
            </a>
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <button
                className="p-2 rounded-full text-slate-600 dark:text-slate-300 hover:text-teal-600 dark:hover:text-teal-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
              ...
              {/* Dark mode toggle */}
              <button
                className="p-2 rounded-full text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
              ...
              {/* XP badge */}
              <div
                className="nav-xp flex items-center gap-1.5 font-mono font-bold text-xs sm:text-sm px-3 py-1.5 rounded-full border border-amber-400/50 bg-amber-400/10 text-amber-500 shadow-sm shadow-amber-400/20"
                data-testid="nav-xp-display"
                title="Total XP"
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="#FFD700" stroke="#FF8C00" strokeWidth="1.2" style={{ filter: 'drop-shadow(0 0 6px rgba(255,215,0,0.8))' }}>
              ...
              {currentUser ? (
                ...
                <div ref={userMenuRef} style={{ position: 'relative' }}>
                  <button
                    ...
                    onMouseLeave={e => e.currentTarget.style.borderColor = 'rgba(245,158,11,0.6)'}
                  >
                    {avatarUrl
                      ? <img src={avatarUrl} alt={displayName} style={{ width: '100%', height: '100%', objectFit: 'cover' }} referrerPolicy="no-referrer" />
                      : <span style={{ fontSize: '.7rem', fontWeight: 800, color: '#F59E0B', letterSpacing: '-.5px' }}>{initials}</span>
                    }
                  </button>
                  ...
                  {userMenuOpen && (
                    <div style={{
                      ...
                      <div style={{
                        ...
                        <div style={{ fontWeight: 700, fontSize: '.88rem', color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                          {currentUser.displayName || 'Veritas Explorer'}
                        </div>
                        <div style={{ fontSize: '.74rem', color: 'var(--text-muted)', marginTop: '.1rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                          {currentUser.email}
                        </div>
                      </div>
          ...
          {/* Mobile dropdown */}
          {mobileOpen && (
            <div className="md:hidden mt-3 pt-3 border-t border-slate-200/60 dark:border-slate-800 flex flex-col gap-1.5">
              {NAV_ITEMS.map(item => {
                const isActive = item.page === activeNavPage;
                return (
                  <a
              ...
              {!currentUser && (
                <button
                  ...
                  className="dock-item"
                  style={{ color: '#F59E0B', fontWeight: 700, textAlign: 'left', background: 'none', border: 'none', cursor: 'pointer' }}
                >
                  Masuk / Daftar
                </button>
              )}
